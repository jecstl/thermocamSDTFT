...
void createTemperatureFile(){
	...
	newFileName = String(random(10000,99999),DEC);
        newFileName += ".thc";
        newFileName.toCharArray(newFileNameChar,(newFileName.length()+1));
...
void setMaxMinT(){		
	res=file.initFAT();
	if (file.exists(currFileNameChar))
	{  
		res=file.openFile(currFileNameChar, FILEMODE_TEXT_READ);
		if (res==NO_ERROR)
		{
			result=0;
...
void drawResult(){	
	...
	myGLCD.print(currFileNameChar,CENTER, 228);
}
